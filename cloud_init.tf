...
resource "null_resource" "moodle1_cloud_init" {
	depends_on = [oci_core_instance.bastion,oci_core_volume_attachment.moodlehtml_attach1]
...
resource "null_resource" "moodle2_cloud_init" {
	depends_on = [oci_core_instance.bastion,oci_core_volume_attachment.moodlehtml_attach2]
	provisioner "file" {
    connection {
		type        = "ssh"
		user        = "opc"
